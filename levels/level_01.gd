...
func _wired():
	...
	m_grp2_switch1.m_transmitter.add_receiver(m_grp2_xor_gate.m_receiver1)
	m_grp2_switch2.m_transmitter.add_receiver(m_grp2_xor_gate.m_receiver2)
	m_grp2_xor_gate.m_transmitter.add_receiver(m_grp1_and_gate.m_receiver1)
	
	#Switch inputs into AndGate and AndGate inputs in Diode
	m_grp1_switch1.m_transmitter.add_receiver(m_grp1_and_gate.m_receiver2)
	m_grp1_and_gate.m_transmitter.add_receiver(m_diode.m_receiver)
